...
The persistent map's default view is **All + IMSR**: every IMSR-grade (significant, still-reporting) fire from the Current point and perimeter services, drawn from the live layer and filtered client-side with `isImsrGradeFire()`. It refreshes on the normal five-minute cycle, so new significant fires appear on their own, and it renders immediately on load without opening the drawer. Complex parents appear as diamonds only when the complex itself is IMSR-grade (the diamonds obey the same `isImsrGradeFire()` filter as the point fires). The member fires of an IMSR-grade complex are drawn with full ignition/perimeter geometry even when an individual member is not itself IMSR-grade (a complex is shown by its fires, not merely its diamond); `renderFireLayers()` builds the set of IMSR complex parent ids and exempts their members from the per-fire IMSR predicate, matching how mirror mode expands a CX row to all its members.

There is no "recently closed" layer. Closed incidents (Contained, Controlled, Out) are reachable only through the database drawer's status filters, which switch the map into mirror mode; they are never part of the default live view.
...
- **Default state** means status `All`, `IMSR` on, empty search, and `Large` off. Sort is deliberately excluded — it reorders rows without changing membership. In the default state the map uses its normal (non-mirror) rendering: the live layer filtered client-side to `isImsrGradeFire()`, showing every match. The default list population (All + IMSR) matches it.
- **Trigger**: `fireFilterStateIsDefault()` is evaluated only after a list page applies successfully (`updateFireMirrorState()`), so an in-flight or failed load never blanks the map, and a late page cannot flip the mode after the wildfire master toggle went off (the function re-checks `wildfiresVisible`). Any deviation from the default — turning `IMSR` off, choosing a non-`All` status, enabling `Large`, or typing a search — engages mirror mode.
- **Mirror set**: `fireMirrorRecords` is maintained at exactly the two points where the list DOM mutates — cleared alongside `fireList.replaceChildren()` and extended as each row is appended — so `Load more` adds map markers in lockstep and an aborted load leaves the mirror untouched. When `IMSR` is on but the state is still non-default (e.g. `Active + IMSR`, or `IMSR + Large`), the list loads its whole set in one page, so the mirror shows every match rather than 50.
- **Rendering**: `renderFireLayers()` branches on `fireMirrorActive`. Non-mirror (the default) skips to the `isImsrGradeFire()` predicate over live events; mirror mode makes the loaded rows *the* visibility set. Feature-to-record lookups go through `fireRenderRecord()` (mirror first, then live events). At render time a mirrored fire prefers the live in-memory record's point and perimeter geometry by normalized IrwinID, so the five-minute WFIGS refresh keeps mirrored geometry current; a historical fire not in memory renders point-only from the list row's `ensurePointGeometry` fallback. Mirrored list attributes themselves are a snapshot of the last user-driven list load — mirror mode adds **zero** recurring WFIGS requests.
- **CX parents in the mirror**: `renderFireLayers()` draws only the member fires found in memory (`complexMemberRecords()`, an in-memory scan matching `CpxID` to the parent id — never a request); the parent itself is never drawn as an ignition circle. The parent is instead represented by its diamond, drawn by `renderComplexMarkers()` from the mirrored CX rows and positioned at the member fires' bounds center (`complexDisplayLatLng()`), falling back to the raw placeholder point only when no member geometry is in memory.
- **Banner**: while mirroring, a `pm-glass` banner under the toolbar shows `Filtered · N fires` with a clear (`×`) button that resets every filter control to the default (All + IMSR) and re-renders; it works with the drawer open or closed. The Layers-menu status line reports the filtered count, and the map's accessible name says "filtered WFIGS wildfire …".
- **Teardown paths** (all four must keep working): restoring the default filters via the chips/search, the banner clear button, the unified manual refresh (`resetMapToInitialState`), and turning the Wildfires master toggle off (`clearFireMirror(false)` so re-enabling shows the default IMSR view). Closing the drawer deliberately does **not** exit mirror mode — that is what the banner is for.
...
24. Confirm the default map view (All + IMSR) draws every IMSR-grade fire from the live layer on first load without opening the drawer, refreshes on the five-minute cycle, and shows a diamond only for IMSR-grade complexes (a non-IMSR complex must not appear). Confirm each diamond sits with its member fires, that the member fires of an IMSR complex draw their ignition/perimeter geometry (not just the diamond) even when individual members are Not current, and that clicking a diamond opens the popup on the diamond (not offset to a dispatch point). Confirm no `Recently closed` control exists and that the Layers menu has exactly four wildfire-related checkboxes (Smoke, Wildfires, Ignitions, Perimeters).
...
36. Exercise the filter/map mirror: default open shows no status chip pressed and `IMSR` pressed, with the map showing every IMSR-grade fire and no banner; turning `IMSR` off, choosing any status chip, enabling `Large`, or typing a search switches the map to exactly the loaded list rows with the `Filtered · N fires` banner (N equals the row count); with `IMSR` off `Load more` grows list and map together and the status reads `N of TOTAL`; with `IMSR` on the whole matching set is shown at once; a nonsensical combination like `Out + IMSR` correctly shows nothing; sort changes membership of neither. Confirm the row-one chips are mutually exclusive and that clicking the pressed chip clears it back to the no-chip "all" state (`Not current` included). Verify all four teardown paths (default restored via chips/search, banner clear with the drawer open and closed, unified refresh, Wildfires master toggle off/on all return to no-status + IMSR) and confirm mirror mode issues no recurring WFIGS requests beyond the user-driven list loads.
