...
- Significant (IMSR-grade) wildfires by default, with the full year-to-date incident database reachable through the drawer's status/size filters.
...
| --- | --- | --- |
| HMS smoke plume polygons | [NOAA HMS Smoke Detection](https://www.arcgis.com/home/item.html?id=ab7a5fbd76e3499296350eabf599fc63) | `NOAA_Satellite_Smoke_Detection_(v1)/FeatureServer/0` on shard `https://services2.arcgis.com/C8EMgrsFcRFL6LrL/arcgis/rest/services` |

This is an ArcGIS Online hosted FeatureServer (polygon geometry, GeoJSON output, `Access-Control-Allow-Origin: *`), the same class of service as WFIGS, so it is fetched and rendered entirely in the browser via `fetchGeojsonFeaturePages` — there is **no** build-pipeline or R2 involvement (unlike the RAQDPS frames). Available fields: `Density` (Light/Medium/Heavy), `Satellite`, `Start`, `End_`. The service publishes the current day's analysis, which contains several intra-day satellite time steps at once.

Because the shard is `services2.arcgis.com` (WFIGS is `services3`), it had to be added to the page `Content-Security-Policy` `connect-src` allowlist. Both `services2` and `services3` are now allowed; a new ArcGIS shard will silently fail with a `TypeError: Failed to fetch` (and no recorded network request) until its host is added there.

### Design and rendering rules

- **Default off.** The overlay loads and renders only after the user ticks the `Observed smoke (HMS)` checkbox. The three top-level layer toggles are ordered `Smoke & PM2.5 (RAQDPS)`, `Wildfires (WFIGS)`, `Observed smoke (HMS)`, each carrying its data source in parentheses, matching the order of the option sections below. Each option section's heading text is identical to its toggle label (`Smoke & PM2.5 (RAQDPS)`, `Wildfires (WFIGS)`, `Observed smoke (HMS)`; the shared section-title style renders them uppercase).
- **Static, all current polygons.** The layer draws every polygon the service currently returns (the whole current-day analysis — the union of today's time steps, so persistent plumes appear as several overlapping polygons). No past/forecast/timeline filtering. This is the deliberate, confirmed design; it matches how NOAA/AirNow present HMS.
- **Density styling.** Graded translucent smoke-grey (`HMS_DENSITY_STYLE`), Light → Heavy by increasing fill opacity, with a dashed outline on Light/Medium so pale fills stay legible on both the light and dark basemaps. Polygons are drawn Light-first, Heavy-last so denser plumes sit on top. The Layers panel shows a matching Light/Medium/Heavy legend and a "N HMS smoke polygons (latest analysis)" status line.
...
**The list, persistent map, and list selection all consume the same canonical snapshot.** Before any replacement page becomes visible, the loader merges the appropriate Current and Year-to-Date membership, resolves Current membership, loads canonical point geometry, hydrates every matching perimeter, and hydrates complex members. Only after all of that succeeds are the list DOM, `fireMirrorRecords`, and `canonicalFireRecords` updated atomically. The default filter is **All + IMSR**, so the default snapshot is the ordered Current ∪ Year-to-Date IMSR set: it includes IMSR fires that have dropped out of Current (for example Alaska monitor fires) and any Current-only carry-over record. The background Current cache (`fireEvents`) may still refresh for service health and legacy refresh bookkeeping, but it is never a render-time geometry fallback and cannot change a canonical point or perimeter independently of its row. The snapshot loads at startup, on every filter/search/sort change, on `Load more`, and on the five-minute cycle in the default state.
...
- Persist at most 12 recently successful database point pages for 24 hours in local storage solely as a stale-on-error fallback. A live response always replaces the stored page; a stored page must be labeled `cached` and used only when live WFIGS is unavailable or rate-limited. The storage key is `na-smoke-map:wfigs-database:v3`; v1 and v2 are removed once at startup because their attribute-only payloads cannot satisfy the canonical-geometry contract.
...
- When `IMSR` is off, show 50 **final matching** records plus a look-ahead and require an explicit `Load more` action. Status filters are finalized only after Current membership has been resolved; pagination is applied to those final matches, never to the broader SQL candidate set. `collectFilteredStatusPage()` automatically scans additional candidate batches until it has 50 canonical matches or exhausts the source. `Not current` uses a 500-candidate scan window because recent no-closure YTD rows are dominated by Current incidents. When `IMSR` is on, scan the whole bounded matching set (`FIRE_IMSR_PAGE_SIZE`, 1000 per candidate batch) and hide `Load more`.
- Show the matching total in the list status when the first page did not already capture everything and the service query is itself the final membership rule (All and Active), e.g. `50 of 24,344 wildfires shown`. `loadFireDatabaseTotal()` runs a `returnCountOnly=true` companion query with the same service and where clause, cached per service+where; it is not awaited, and a failed count silently leaves the plain `N shown`. Contained, Controlled, Out, and Not current show a plain `N shown` because their final membership is validated through `fireStatus()` after Current membership resolution, so the SQL candidate count is not guaranteed exact.
...
- No status chip ("all") merges the matching Year-to-Date rows with matching Current-only rows in the requested server sort order. Contained, Controlled, Out, and Not current query Year-to-Date only.
...
- Not current means no official end date plus either (a) absence from the live/current service or (b) a final (`F`) ICS-209 report, whose priority intentionally wins even if WFIGS still retains the row in Current. Current membership is not a Year-to-Date field, so this filter cannot be pure SQL: SQL narrows to no-closure candidates, `collectFilteredStatusPage()` resolves Current membership, applies the shared `fireMatchesDatabaseStatus()`/`fireStatus()` rule, and continues scanning until the visible page is full. This deliberately includes still-reporting U/I fires that have dropped out of Current, such as Alaska monitor fires, while never exposing an empty recent candidate page as an empty final result.
...
- Cancel obsolete search, filter, sort, pagination, total-count, and HMS requests with `AbortController` and generation counters. Selection has no request to cancel. Late snapshot results must not overwrite the latest user action.
...
- Database point-page queries use `f=geojson`, `returnGeometry=true`, and `outSR=4326`. Candidate point rows resolve Current membership and final filter membership before expensive perimeter/complex hydration, so rejected candidates never download polygon geometry. Only final visible records are hydrated before commit. `InitialLongitude`/`InitialLatitude` remains only the fallback when the service feature itself lacks point geometry.
...
- **Atomic snapshot commit**: `loadFireDatabase()` obtains point candidates, merges Current-only records when the filter is All, resolves Current membership, applies the final shared filter, and completes source-matched perimeter/complex hydration before mutating visible state. Broad scans reject candidates before polygon hydration; the small default IMSR path uses its one full perimeter pass to supply both geometry and missing filter attributes, then rejects any contradiction before complex hydration and commit. A reset keeps the old list/map until the replacement is complete. On success, `fireList`, `fireMirrorRecords`, and `canonicalFireRecords` are cleared and extended together; `Load more` extends all three together.
- **Unified membership and pagination**: All uses a two-stream ordered merge of Year-to-Date rows and the small Current-only difference. The unpaginated All + IMSR path obtains both filtered streams together and derives their overlapping membership directly. Active reads Current directly. Closed and Not current read Year-to-Date. Other paths use one lightweight `returnIdsOnly` OBJECTID membership snapshot per needed Current/YTD service, cached for five minutes; shared OBJECTIDs resolve membership locally, with IRWIN fallback only when an OBJECTID is absent. Status-filter candidate scanning buffers surplus final matches so every non-IMSR page contains up to 50 real matches in requested sort order; raw candidate offsets never become visible-result offsets.
...
- **Teardown paths** (all must keep working): restoring the default filters via the chips/search, the banner clear button, the unified manual refresh (`resetMapToInitialState`, which reloads the default list), and turning the Wildfires master toggle off clear both canonical maps and the Current-only merge cursor. Re-enabling Wildfires reloads the default canonical snapshot even if the background Current refresh is still fresh. Closing the drawer does **not** change the map.
...
- The default startup performs **only** the canonical All + IMSR snapshot load. Do not start the legacy full-Current point/perimeter refresh in parallel: that duplicate cold-start path previously competed for the same ArcGIS organization quota, delayed the first visible fires, and could turn an otherwise successful canonical query into a 429 failure. The five-minute/visibility cycle refreshes the canonical snapshot directly instead of downloading the unused full-Current cache first. The legacy Current cache remains page-memory-only for explicit unified full-refresh health/bookkeeping; it is never consulted while rendering a canonical record.
- Cache at most 24 successful canonical point pages for five minutes and evict the oldest entry at the limit. Also cache up to 1,500 per-record canonical perimeter results (including known-empty results) and 100 complex-member point sets for five minutes so repeated filter/sort changes do not download identical geometry. The only browser-persistent WFIGS data is the bounded 12-page, 24-hour v3 point-page fallback described above; never treat it as live data.
- Resolve ordinary Current/YTD membership from one cached `returnIdsOnly` snapshot per service rather than issuing one objectIds query per candidate batch. This reduces repeated filter membership to in-memory set lookups. `fetchArcgis()` automatically switches query parameters to a form-encoded POST when a GET URL would exceed 1,800 characters; the services3 front end otherwise returns an HTML 404 for long IrwinID `IN` URLs. Perimeter/member queries therefore use up to 500 identifiers per POST-capable batch (verified live) and `mapWithConcurrency()` at a hard maximum of two same-host requests. YTD and Current perimeter hydration may run together; complex source hydration may run together.
- The startup/default All + IMSR path queries the already-selective IMSR SQL against Current and Year-to-Date **in parallel**, builds their sorted union directly, and derives Current membership from that pair. It does not download the unfiltered Current location service and does not request service-wide membership snapshots for top-level rows. For non-IMSR All views, the Current-only helper also pushes the active search/size modifiers into its service query rather than downloading all Current records before filtering.
- Apply final status/IMSR membership before polygon hydration for broad paginated/status scans. Because perimeter attributes can supply an end date missing from the point row, those candidates first receive a lightweight no-geometry perimeter-attribute pass (cached for five minutes); only accepted visible records download polygon geometry. The small server-filtered All + IMSR startup set is the deliberate exception: one full perimeter pass supplies both canonical geometry and any missing end-state attributes, then the final client-side IMSR check runs before complex-member hydration. This removes the former duplicate attribute-plus-polygon query for the same startup records without changing final membership. A `Not current` page scans 500 point candidates at a time and buffers surplus matches.
- On first background refresh, fetch full display-simplified Current perimeter GeoJSON directly, avoiding a fragile index-plus-geometry request burst.
- On later background refreshes, fetch a lightweight `OBJECTID`, polygon-time, and modified-time index, reuse unchanged geometry from `firePerimeterCache`, request only new or changed object IDs, and remove records no longer returned by Current. This cache can trigger/default-refresh bookkeeping but cannot overwrite canonical render geometry.
- If the incremental background perimeter path fails, retry a full perimeter query. Retain the prior canonical snapshot until `loadFireDatabase()` completes a fully hydrated replacement.
- A partial background refresh must still attempt the default All + IMSR canonical reload when that list was reset; partial cache availability is not permission to blank or partially mutate the map.
...
- Use a 45-second refresh controller timeout, request generation checks, sequential ordered point-page scans, and bounded same-host hydration concurrency.
- Canonical snapshot loads are atomic: do not expose point rows before their perimeter/member hydration completes.
- Refresh the default canonical snapshot directly while the page is visible every five minutes. On `pageshow` or visibility resume, do not start a second canonical or explicit full refresh while either controller is already running. A non-default filtered/paginated view is not automatically replaced.
- Preserve the old wildfire layer on any refresh failure and expose a concise Layers-menu error plus `Retry`.

The map's manual refresh button is a unified refresh-and-reset action. It must refetch the smoke cache manifest and every field atlas for the initial wildfire-smoke/surface dataset, refresh the background Current WFIGS point/perimeter cache, rebuild the default canonical Current + Year-to-Date snapshot, and restore the opening map state: North America center and zoom, Day basemap, wildfire-smoke surface PM2.5 at Now, stopped playback, Smoke/Wildfires/Ignitions/Perimeters on, closed menus and database drawer, default database search/filter/sort state (status `All`, `IMSR` on, `Large` off) with the filtered banner hidden, the default list reloaded even while the drawer is closed so the map repopulates, and no popup or selected wildfire. A known current-location dot may remain, matching startup behavior after location permission has already been granted. Clear the database page cache and mark the visible list for atomic replacement before that reload. Its accessible name is `Refresh smoke and wildfire data`. Report success only if both primary systems succeed, partial success if one succeeds, and retain the prior visible smoke frame or canonical wildfire snapshot for any failed source; if the default smoke reload fails, restore its prior particle, extent, and hour labels so they continue to match the retained frame.
...
21. Confirm every Current and Year-to-Date WFIGS request contains its correct WF category filter and that no RX event appears anywhere. Confirm CX records appear only where documented: as database roll-up rows (with the `Complex` badge, no complex-child rows alongside), database selections, and as diamonds with their member-fire geometry — a CX parent is never drawn as a WF ignition circle.
...
24. Confirm the default map view (All + IMSR) populates on first load without opening the drawer and that **the map count equals the drawer list count**, including IMSR fires that have left Current (such as Kilolitna) and any matching Current-only carry-over incident. Confirm the five-minute refresh keeps the default canonical set current, complex diamonds sit with their hydrated member fires, hovering a diamond emphasizes it (grow + pointer cursor) exactly like an ignition circle or perimeter, and clicking a diamond opens the popup on the diamond. Confirm no `Recently closed` control exists and that the Layers menu has the documented layer checkboxes: three top-level toggles in order — `Smoke & PM2.5 (RAQDPS)`, `Wildfires (WFIGS)`, `Observed smoke (HMS)`, each carrying its data source in parentheses — plus the two wildfire-geometry checkboxes (Ignitions, Perimeters), with the HMS toggle unchecked by default. Confirm each option section's heading text matches its toggle label exactly.
...
28. Open the wildfire database on desktop and phone; test at least ten fresh early-page opens, 300 ms name search, all status filters, the independent 300+ acre filter, the IMSR toggle, newest-first and acreage-first ordering, and rapid consecutive actions whose stale responses arrive late. Confirm that with IMSR off the list paginates by 50 **final** matches, automatically scanning past rejected SQL candidates without an empty first page; All/Active may read `N of TOTAL`, while the post-membership status filters read a plain `N shown`. Confirm All is the correctly sorted YTD + Current-only union; `Not current` alone fills its first page when at least 50 matches exist; `Not current + IMSR` lists still-reporting fires that have left Current; IMSR hides `Load more` after scanning the whole bounded set; cached repeat queries return immediately; and a failed reset retains the old atomic snapshot without duplicate Retry results.
...
31. Test a fresh WFIGS load, five-minute/visibility refresh, Layers-menu Retry, and the unified manual smoke-and-wildfire refresh. Before manual refresh, change viewport/zoom, basemap, particle/extent/hour, playback state, layer switches, popup/selection, database filters/search/sort, and open panels. Confirm refresh restores the documented opening state, refetches every default smoke atlas plus a complete default canonical WFIGS snapshot, preserves an already-known location dot, has no startup `pageshow` race or duplicate active refresh, uses only the valid `services3` host, retains the prior canonical snapshot on a replacement failure, and reports success/partial/error feedback matching the actual two-source result. Click both map utility buttons over probeable pollution and confirm neither click opens a PM2.5 popup; in particular, switch to Total PM2.5 before refresh and confirm no stale Total PM2.5 probe or popup auto-pan overrides the opening viewport.
32. Inspect WFIGS request count and payload behavior: on fresh startup, exactly the selective All + IMSR Current/YTD point pair (not an unfiltered Current point download and not the legacy full-Current perimeter refresh), direct top-level membership derivation without service-wide ID snapshots, and one full perimeter pass before final IMSR validation; on broad paginated/status paths, GeoJSON point candidates with final filtering before polygon hydration and at most one five-minute `returnIdsOnly` membership snapshot per needed Current/YTD service. Confirm automatic form-encoded POST beyond 1,800 URL characters; perimeter/member batches no larger than 500 IrwinIDs; at most two concurrent independent hydration requests; pre-commit complex-member hydration; zero selection-time requests; five-minute point/membership/perimeter/complex cache reuse including cached empty perimeter results; 24-page point eviction, 1,500 perimeter-result eviction, 100 complex-set eviction; explicit cache clearing during unified manual refresh; and bounded 12-page v3 persistent fallback retention.
...
38. Exercise the NOAA HMS observed-smoke overlay. Confirm it is off on first load and draws nothing until ticked. On enabling, confirm exactly one page of GeoJSON polygons loads from the `services2.arcgis.com` HMS service, they render as graded grey Light/Medium/Heavy plumes (dashed outline on the paler classes) in `hmsPane` beneath the WFIGS fires and above the modeled smoke canvas, the Layers panel shows the disclaimer note, the Light/Medium/Heavy legend, and the `N HMS smoke polygons (latest analysis)` status, and the `NOAA HMS Smoke` attribution appears. Confirm the layer does not react to the timeline slider, playback, or particle/extent changes. Toggle it off and confirm the polygons, options section, and HMS attribution are removed while the WFIGS attribution is untouched; toggle back on and confirm the cached data re-renders instantly. Also toggle on/off/on before the first request finishes and confirm the superseded request cannot redraw or overwrite the final successful layer. Simulate a load failure and confirm the status shows an error with a working Retry. Confirm tooltip text treats the Density field as text, and the `connect-src` CSP allowlist still contains both `services2.arcgis.com` and `services3.arcgis.com`.
...
- Direct WFIGS loading avoids maintaining another publication pipeline and keeps incidents timely, but availability depends on an official ArcGIS organization-wide quota. Batched hydration, five-minute page reuse, quota-aware retry, atomic old-snapshot retention, and a clearly labeled bounded stale point-page fallback mitigate transient failures. A stale point page still needs live membership/perimeter hydration before it can replace the visible canonical snapshot; if that hydration fails, retain the prior snapshot. The persistent point-page fallback may be up to 24 hours old and must never be presented as live data.
