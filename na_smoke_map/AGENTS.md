...
Because HMS and WFIGS are now cache-only in the browser, the page `Content-Security-Policy` `connect-src` allowlist is `'self'` only. Adding any future browser-side source requires explicitly allowing its host, but build-time sources do not belong in browser CSP.
