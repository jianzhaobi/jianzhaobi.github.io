...
The Current services supply the normal live map (the default IMSR view). The Year-to-Date services supply the incident database and on-demand geometry for a database selection. The database intentionally covers only the current calendar year in the first implementation; do not imply that it is an all-years archive.

Every WFIGS request must enforce the official wildfire categories:

- Live map ignition/perimeter geometry stays WF-only: `IncidentTypeCategory='WF'` for points and `attr_IncidentTypeCategory='WF'` for perimeters. A CX parent is never drawn as a normal WF ignition circle or perimeter. Member fires (`IsCpxChild = 1`) are WF and *are* not filtered out of these queries, so they render as normal live fires.
- CX parents render as a distinct **complex diamond marker** at the center of their member fires' geometry. `fetchComplexPointFeatures()` fetches `IncidentTypeCategory='CX'` points from the Current location service into `currentComplexPoints` (separate from `fireEvents`, so WF-only ignition/perimeter rendering is untouched); `renderComplexMarkers()` draws one diamond per parent, positioned by `complexDisplayLatLng()` (the bounds center of the in-memory member fires, falling back to the raw CX point only when no member geometry resolves). This is the only way a CX parent appears on the persistent live map — as a diamond, never as a WF fire.
- Database list queries use the complex roll-up clause `((IncidentTypeCategory='WF' AND (IsCpxChild = 0 OR IsCpxChild IS NULL)) OR IncidentTypeCategory='CX')` so each complex appears as one CX parent group-header row; its member fires (`IsCpxChild = 1`) are excluded from the paginated list and instead fetched on demand when the header is expanded (`IsCpxChild=1 AND CpxID='<parent IrwinID>'`). Database selection and selection-perimeter queries may use `IN ('WF','CX')` so a CX parent row can resolve its geometry.
...
**The map always mirrors the loaded database list — there is a single source of truth for which fires appear.** The rows the list returns for the current filter are exactly the fires drawn, in every state including the default. The default filter is **All + IMSR**, so the default map/list is every IMSR-grade (significant, still-reporting) fire. The list query runs against the Year-to-Date service, so the default view includes IMSR fires that have dropped out of the live Current feed (e.g. Alaska monitor fires) — the map and the drawer can never disagree on membership. The live Current layer (`fireEvents`) is retained only as the geometry/perimeter provider: at render time each list row prefers the live in-memory point and perimeter by normalized IRWIN so the five-minute refresh keeps geometry current, and a row not in the Current feed renders point-only from its reported coordinates. Complex parents appear as diamonds (from the list's CX rows); their member fires draw from live geometry. The list set is loaded on startup, on every filter/search/sort change, on `Load more`, and refreshed on the five-minute cycle (default state only, to avoid disturbing an active filtered/paginated view), so new significant fires still appear on their own.
...
- When `IMSR` is off, fetch 50 visible records plus one look-ahead record and require an explicit `Load more` action. When `IMSR` is on, fetch the whole matching set in one request (`FIRE_IMSR_PAGE_SIZE`, 1000) and hide `Load more` — IMSR is a bounded set (tens to low hundreds), so "shows every match" is safe and expected.
- Show the matching total in the list status when the first page did not already capture everything, e.g. `50 of 24,344 wildfires shown`. `loadFireDatabaseTotal()` runs a `returnCountOnly=true` companion query with the same service and where clause, cached per service+where; it is not awaited (the list shows immediately and the `of N` clause fills in when the count returns), and a failed count silently leaves the plain `N shown`. Its dedicated controller is independent of row-loading/selection requests and is aborted on a new reset, drawer close, reset, or page hide; `resumeFireDatabaseTotalIfNeeded()` restarts an eligible aborted count when the populated drawer opens or the page returns. `renderFireListStatus()` composes the line so it can be re-run when the count resolves, including when the default list loaded while the drawer was closed.
- Debounce incident-name search by approximately 300 ms and escape SQL quote characters.
- Row one is Active, Contained, Controlled, Out, and Not current — mutually exclusive status chips. There is no `All` chip: **no chip pressed is the "all statuses" state**, and clicking the already-pressed chip clears it back to that state (`fireDatabaseFilter === "all"`). Row two holds the independent `IMSR` and `Large · 300+ acres` toggles (in that order). **The default is no status chip + `IMSR` on**, so the default list population is every IMSR-grade fire and matches the map's default live view. `IMSR` is a pure modifier: it ANDs with the row-one status. `Contained/Controlled/Out + IMSR` return (almost) nothing (a closed fire is rarely still IMSR-reporting), but `Not current + IMSR` is meaningful — it lists IMSR-grade fires that have dropped out of the Current feed (e.g. Alaska monitor fires).
...
- No status chip ("all"), Contained, Controlled, Out, and Not current query the Year-to-Date location service.
...
- Not current means "no closure dates **and** not in the live/current service" — the same thing `fireStatus()` badges `Not current`. Current membership is not a Year-to-Date field, so this filter cannot be pure SQL: the query narrows to no-closure records (`ContainmentDateTime IS NULL AND ControlDateTime IS NULL AND FireOutDateTime IS NULL`) and `loadFireDatabase()` then keeps only the fetched rows whose `fireStatus()` is actually `Not current`, dropping any that are currently active. This deliberately includes still-reporting (U/I) fires that have dropped out of the Current feed — e.g. Alaska monitor fires like Kilolitna — so `Not current` **does** combine with `IMSR` (an earlier `ICS209ReportStatus = 'F'` definition wrongly returned zero there, since `F` contradicts IMSR's `U`/`I`). Because the client-side pass removes rows, the `of N` total is not shown for `Not current` (a SQL count would include the active rows that get removed); the status reads a plain `N shown`.
- Every list where clause starts from the complex roll-up base described in the official-sources section; a CX parent renders as a collapsible **group header** carrying an outline `Complex` badge next to the status badge. The header body is a button that runs `zoomToComplex()` (framing all member fires); a trailing chevron button (`aria-expanded`) lazily fetches the complex's member fires on first expand (`IsCpxChild=1 AND CpxID='<parent IrwinID>'`, `f=json`, `returnGeometry=false`, against the same Current/Year-to-Date service as the parent row) and nests them as ordinary selectable rows. Member loads cache on the row (no refetch on re-expand), show a loading line, offer a Retry on failure, and are aborted on drawer close and list reset via `abortComplexMemberLoads()`. Selecting a member row behaves like any fire selection. `buildFireRowButton()` is the shared row markup for standalone fires, members, and the header.
- Contained means containment is present while control and out are absent. Controlled means control is present while out is absent. Out means out is present. Not current means a final ICS-209 report with none of those three dates.
- Cancel obsolete search, filter, sort, pagination, selection, total-count, and HMS requests with `AbortController` and generation counters. Late results must not overwrite the latest user action.
...
The drawer's filter state and the map display are unified in one direction: **the map always draws exactly the rows loaded in the list**, for every filter and for no filter. `fireMirrorRecords` is the single source of what is drawn; there is no separate "live active" rendering path. This guarantees the map and the drawer can never disagree on membership.

- **List set (`fireMirrorRecords`)** is maintained at the two points where the list DOM mutates — cleared alongside `fireList.replaceChildren()` and extended as each row is appended — so `Load more` adds map markers in lockstep and an aborted load leaves it untouched. Normalize and de-duplicate an IRWIN id before appending its row so an ArcGIS page-boundary repeat cannot make the drawer count differ from the map count. The set is loaded on startup (`initialize()` calls `loadFireDatabase(true)` even with the drawer closed — the old `fireDrawer.hidden` early-return in `loadFireDatabase()` was removed so the map can populate/refresh without the drawer), on every filter/search/sort change, on `Load more`, and re-loaded on the five-minute cycle **only when `fireFilterStateIsDefault()`** (a `refreshWildfires()` success or partial-geometry result reloads the default list; non-default filtered/paginated views keep the user's snapshot so their pagination is not disrupted — their geometry still refreshes because `renderFireLayers()` runs on every refresh).
- **Rendering**: `renderFireLayers()` always iterates `fireMirrorRecords`. Feature-to-record lookups go through `fireRenderRecord()` (list row first, then live record). Each row prefers the live in-memory point and perimeter geometry by normalized IRWIN (via `fireEvents`), so the five-minute WFIGS refresh keeps geometry current; a row not in the Current feed renders point-only from its reported coordinates (`ensurePointGeometry`). With IMSR on, the list loads its whole set in one page (so no `Load more`); other filters page 50.
- **CX parents**: `rebuildFireEvents()` creates the `fireComplexMembers` index keyed by normalized parent IRWIN, and `renderFireLayers()` draws only the indexed member fires returned by `complexMemberRecords()` (never a request or a repeated whole-event scan); the parent itself is never drawn as an ignition circle. It is represented by its diamond, drawn by `renderComplexMarkers()` from the list's CX rows and positioned at the member fires' bounds center (`complexDisplayLatLng()`), falling back to the raw placeholder point only when no member geometry is in memory.
...
- **Teardown paths** (all must keep working): restoring the default filters via the chips/search, the banner clear button, the unified manual refresh (`resetMapToInitialState`, which reloads the default list), and turning the Wildfires master toggle off (`clearFireMirror(false)` so re-enabling shows the default). Re-enabling Wildfires must reload a reset list even if live geometry is still inside its five-minute freshness window; otherwise a recently filtered map can return as an empty default view. Closing the drawer does **not** change the map.
- **Complex current-membership**: a CX parent is never in `fireEvents` (WF-only), so `fireRecordFromPointFeature()` also treats a CX record as active when its normalized IRWIN is in `currentComplexIds` — the set of complexes present in the Current service, rebuilt in `refreshWildfires()` alongside `currentComplexPoints`. This keeps an actively reporting complex from falling through to a misleading `Not current` badge (a final-report complex still badges `Not current` because the `fireStatus` `'F'`-first rule wins). The `fireStatus` rules themselves are unchanged.
...
- Keep live points and perimeter geometry in page memory. Cache at most 24 successful database pages for five minutes and evict the oldest entry at the limit. The only browser-persistent WFIGS data is the bounded 12-page, 24-hour database fallback described above; never treat it as live data.
- On first load, fetch full display-simplified Current perimeter GeoJSON directly, avoiding a fragile index-plus-geometry request burst.
...
- A partial geometry refresh must still reload the default All + IMSR list when that list was reset; partial perimeter/complex availability is not permission to leave the map membership empty.
...
24. Confirm the default map view (All + IMSR) populates on first load without opening the drawer and that **the map count equals the drawer list count** (e.g. both read the same "N significant (IMSR) wildfires shown" / "N IMSR wildfires shown"), including the IMSR fires that have left the Current feed (Alaska monitor fires such as Kilolitna must appear as points on the default map, not only in the list). Confirm the five-minute refresh keeps the default set current, complex diamonds sit with their member fires and their members draw geometry, hovering a diamond emphasizes it (grow + pointer cursor) exactly like an ignition circle or perimeter, and clicking a diamond opens the popup on the diamond. Confirm no `Recently closed` control exists and that the Layers menu has the documented layer checkboxes: three top-level toggles in order — `Smoke & PM2.5 (RAQDPS)`, `Wildfires (WFIGS)`, `Observed smoke (HMS)`, each carrying its data source in parentheses — plus the two wildfire-geometry checkboxes (Ignitions, Perimeters), with the HMS toggle unchecked by default. Confirm each option section's heading text matches its toggle label exactly.
...
28. Open the wildfire database on desktop and phone; test at least ten fresh early-page opens, 300 ms name search, all status filters, the independent 300+ acre filter, the IMSR toggle, newest-first and acreage-first ordering, and rapid consecutive actions whose stale responses arrive late. Confirm that with IMSR off the list paginates 50 at a time with `Load more` and the status reads `N of TOTAL wildfires shown` once the count resolves (except `Not current`, which is client-side filtered and reads a plain `N shown`); that `Not current` + IMSR lists the still-reporting fires that have left the Current feed (e.g. Kilolitna) rather than nothing; that with IMSR on the whole set loads in one request with `Load more` hidden and the status reads a plain `N`; that cached repeat queries return immediately; that attribute-only JSON records resolve to canonical point geometry on selection (using reported initial coordinates only as a failed-query fallback); and that a failed reset retains existing rows without duplicate Retry results.
29. Select active, closed-status (Contained/Controlled/Out), older Not current, point-only, perimeter-only, and multi-polygon database events. Confirm the map stops any previous animation, centers an available ignition in the padded usable viewport, keeps every matching perimeter visible at no more than zoom 15, and does not shift after the popup opens. Confirm point-only selection flies to zoom 13, selected archived ignitions use a neutral gray-brown fill and white selection border without adopting the active age palette, the phone sheet closes, otherwise-hidden incidents draw temporarily, and missing geometry reports `Location unavailable` without moving.
...
36. Exercise the filter/map mirror. The map must always equal the loaded list: in every state the map markers are exactly the list rows (verify the map count equals the list count as filters change). Default open shows no status chip pressed and `IMSR` pressed, the map showing the full IMSR set and no banner; turning `IMSR` off, choosing any status chip, enabling `Large`, or typing a search updates the map to exactly the loaded list rows with the `Filtered · N fires` banner (N equals the row count); with `IMSR` off `Load more` grows list and map together and the status reads `N of TOTAL` (except `Not current`, plain `N`); with `IMSR` on the whole matching set is shown at once; `Out + IMSR` shows the two contradictory records, `Not current + IMSR` shows the fires that left the Current feed; sort changes membership of neither. Confirm the row-one chips are mutually exclusive and that clicking the pressed chip clears it back to the no-chip "all" state. Verify all teardown paths (default restored via chips/search, banner clear with the drawer open and closed, unified refresh, Wildfires master toggle off/on all return to no-status + IMSR with the map repopulated, not blank).
37. Confirm the popup's `ICS-209 report` row appears for any fire with a valid report time and is absent otherwise, and that a complex member fire shows `Part of <CpxName>`. Select a CX parent row and confirm the selection prefers fitted perimeter bounds (CX perimeters resolve through the widened `IN ('WF','CX')` selection queries); its point-only fallback may sit at the documented placeholder dispatch coordinate, which is source data, not a rendering bug. On the map confirm a CX parent draws its in-memory member fires and a diamond at their center, never a circle at the placeholder parent point.
