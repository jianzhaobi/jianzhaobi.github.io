...
- Contained means containment is present while control and out are absent. Controlled means control is present while out is absent. Out means out is present. Not current means no closure dates and no Current membership, subject to the final-ICS-209 priority rule.
...
- Database point-page queries use `f=geojson`, `returnGeometry=true`, and `outSR=4326`. Each page is then hydrated before commit: resolve YTD records' Current membership, load every matching perimeter from the record's source view, and preload complex members with their points and perimeters. `InitialLongitude`/`InitialLatitude` remains only the fallback when the service feature itself lacks point geometry.
...
- **Atomic snapshot commit**: `loadFireDatabase()` obtains the point page, merges Current-only records when the filter is All, resolves Current membership, hydrates source-matched perimeters, and hydrates complex members before mutating any visible state. A reset keeps the old list/map until the replacement is complete. On success, `fireList`, `fireMirrorRecords`, and `canonicalFireRecords` are cleared and extended together; `Load more` extends all three together. Normalized IRWIN identity de-duplicates ArcGIS page-boundary repeats. Startup and the default five-minute refresh run the same loader even while the drawer is closed.
- **Unified membership and pagination**: All uses a two-stream ordered merge of Year-to-Date rows and the small Current-only difference, preserving discovery/acreage sort order and 50-row pagination. Active reads Current directly. Closed and Not current read Year-to-Date; compact shared-`OBJECTID` Current lookups (with IRWIN fallback when an OBJECTID is missing) make `record.active` explicit before status/filter evaluation. Totals for All add the Current-only count.
...
28. Open the wildfire database on desktop and phone; test at least ten fresh early-page opens, 300 ms name search, all status filters, the independent 300+ acre filter, the IMSR toggle, newest-first and acreage-first ordering, and rapid consecutive actions whose stale responses arrive late. Confirm that with IMSR off the list paginates 50 at a time with `Load more` and the status reads `N of TOTAL wildfires shown` once the count resolves (except `Not current`, which is client-side filtered and reads a plain `N shown`); that All is the correctly sorted union of YTD plus Current-only records; that `Not current` + IMSR lists the still-reporting fires that have left Current (e.g. Kilolitna); that with IMSR on the whole set loads in one request with `Load more` hidden and the status reads a plain `N`; that cached repeat queries return immediately; and that a failed reset retains the existing atomic snapshot without duplicate Retry results.
...
32. Inspect WFIGS request count and payload behavior: GeoJSON point pages with geometry, compact Current-membership checks, bounded IrwinID perimeter batches, pre-commit complex-member hydration, zero selection-time geometry requests, Current-only/YTD ordered merging, five-minute database-page reuse, 24-page eviction, explicit memory-cache clearing during unified manual refresh, and bounded 12-page v3 persistent fallback retention.
...
36. Exercise the filter/map mirror. The map must always equal the loaded list: in every state the top-level markers/groups are exactly the list rows and use the same canonical geometry (verify counts and representative coordinates/perimeters as filters change). Default open shows no status chip pressed and `IMSR` pressed, the map showing the full Current ∪ YTD IMSR set and no banner; turning `IMSR` off, choosing any status chip, enabling `Large`, or typing a search updates the map to exactly the loaded list rows with the `Filtered · N fires` banner (N equals the row count); with `IMSR` off `Load more` grows list and map together and the status reads `N of TOTAL` (except `Not current`, plain `N`); with IMSR on the whole matching set is shown at once; `Not current + IMSR` shows the fires that left Current; sort changes membership of neither. Confirm the row-one chips are mutually exclusive and that clicking the pressed chip clears it back to the no-chip "all" state. Verify all teardown paths (default restored via chips/search, banner clear with the drawer open and closed, unified refresh, Wildfires master toggle off/on all return to no-status + IMSR with the map repopulated, not blank).
