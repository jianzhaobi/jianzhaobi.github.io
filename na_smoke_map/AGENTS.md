...
Because HMS still loads live in the browser, `services2.arcgis.com` remains in the page `Content-Security-Policy` `connect-src` allowlist. WFIGS is fetched only by the hourly build pipeline, so `services3.arcgis.com` is deliberately absent from browser `connect-src`; adding another browser-side ArcGIS source still requires explicitly allowing its shard.
...
- Browser CSP now allows the live `services2.arcgis.com` HMS source but deliberately excludes WFIGS's `services3.arcgis.com`. Static regression tests enforce the cache-only startup/filter/refresh paths and the workflow builder step.
...
31. Test a fresh wildfire-cache load, hourly/visibility refresh, Layers-menu Retry, and the unified manual smoke-and-wildfire refresh. Before manual refresh, change viewport/zoom, basemap, particle/extent/hour, playback state, layer switches, popup/selection, database filters/search/sort, and open panels. Confirm refresh restores the documented opening state, refetches every default smoke atlas plus the latest wildfire manifest/default asset, preserves an already-known location dot, terminates a superseded catalog Worker, rejects late old-version commits, retains the prior canonical snapshot on a replacement failure, and reports success/partial/error feedback matching the actual two-cache result. Click both map utility buttons over probeable pollution and confirm neither click opens a PM2.5 popup; in particular, switch to Total PM2.5 before refresh and confirm no stale Total PM2.5 probe or popup auto-pan overrides the opening viewport.
