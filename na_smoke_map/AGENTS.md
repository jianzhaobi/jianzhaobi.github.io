...
- Row one is Active, Contained, Controlled, Out, and Not current — mutually exclusive status chips. There is no `All` chip: **no chip pressed is the "all statuses" state**, and clicking the already-pressed chip clears it back to that state (`fireDatabaseFilter === "all"`). Row two holds the independent `IMSR` and `Large · 300+ acres` toggles (in that order). **The default is no status chip + `IMSR` on**, so the default list population is every IMSR-grade fire and matches the map's default live view. `IMSR` is a pure modifier: it ANDs with the row-one status, so `Contained/Controlled/Out/Not current + IMSR` correctly return (almost) nothing.
...
- Not current targets the primary `Not current` case in `fireStatus()` — reporting has ended without an official end date: `ICS209ReportStatus = 'F' AND ContainmentDateTime IS NULL AND ControlDateTime IS NULL AND FireOutDateTime IS NULL`. Requiring the final (`'F'`) report keeps still-reporting (U/I) active fires out of this bucket. This is intentionally narrower than every fire the map might badge `Not current` (it omits YTD records that dropped out of Current with no report at all); those still appear under the no-chip "all" view.
...
28. Open the wildfire database on desktop and phone; test at least ten fresh early-page opens, 300 ms name search, all status filters, the independent 300+ acre filter, the IMSR toggle, newest-first and acreage-first ordering, and rapid consecutive actions whose stale responses arrive late. Confirm that with IMSR off the list paginates 50 at a time with `Load more` and the status reads `N of TOTAL wildfires shown` once the count resolves; that with IMSR on the whole set loads in one request with `Load more` hidden and the status reads a plain `N`; that cached repeat queries return immediately; that attribute-only JSON records resolve to canonical point geometry on selection (using reported initial coordinates only as a failed-query fallback); and that a failed reset retains existing rows without duplicate Retry results.
