...
- **Atomic snapshot commit**: `loadFireDatabase()` obtains point candidates, merges Current-only records when the filter is All, resolves Current membership, applies the final shared filter, then hydrates only accepted source-matched perimeters and complex members before mutating visible state. A reset keeps the old list/map until the replacement is complete. On success, `fireList`, `fireMirrorRecords`, and `canonicalFireRecords` are cleared and extended together; `Load more` extends all three together.
- **Unified membership and pagination**: All uses a two-stream ordered merge of Year-to-Date rows and the small Current-only difference. Active reads Current directly. Closed and Not current read Year-to-Date. Each Current/YTD service supplies one lightweight `returnIdsOnly` OBJECTID membership snapshot cached for five minutes; shared OBJECTIDs resolve membership locally, with IRWIN fallback only when an OBJECTID is absent. Status-filter candidate scanning buffers surplus final matches so every non-IMSR page contains up to 50 real matches in requested sort order; raw candidate offsets never become visible-result offsets.
...
- Compute the small Current-only difference once per five-minute window from one unfiltered Current top-level point snapshot plus the cached YTD OBJECTID set. Search, Large, IMSR, and sort are then applied locally to that already-small difference, so changing an All-view modifier never repeats the full Current-vs-YTD membership download.
- Apply final status/IMSR membership before polygon hydration. Because perimeter attributes can supply an end date missing from the point row, candidate records first receive a lightweight no-geometry perimeter-attribute pass (cached for five minutes); only accepted visible records download polygon geometry. A `Not current` page scans 500 point candidates at a time and buffers surplus matches.
