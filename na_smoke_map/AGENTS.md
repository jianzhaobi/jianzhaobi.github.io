...
The first source is BC Wildfire Service's current `Fire Locations` layer. The hourly builder fetches only `FIRE_NUMBER` and `INCIDENT_NAME`, normalizes the CWFIS and BC fire numbers by stripping punctuation, case, and an optional leading four-digit year, and joins only exact BC-to-BC identifiers. It accepts a nonblank `INCIDENT_NAME` only when it differs from `FIRE_NUMBER`; BC frequently repeats the identifier in `INCIDENT_NAME`, which is not a name. Conflicting duplicate official names for one normalized ID are skipped. The serialized record stores the accepted name and `BC Wildfire Service` provenance, and a Canadian popup exposes that provenance in a `Name source` row. The manifest records BC source, fetched record count, usable-name count, exact matched-name count, and skipped ambiguous-ID count.
...
- `scripts/build_canada_wildfire_cache.py` runs hourly in the Pages workflow and publishes `cache/canada-wildfires/manifest.json` plus content-addressed `default` and `catalog` JSON assets. The default asset is the matched CIFFC Priority subset; the catalog is the complete temporally current Agency Reported set. It also fetches BC's official name layer on every build, so a failure of CWFIS, CIFFC, or BC retains a complete prior cache rather than publishing a partial name snapshot. The manifest distinguishes CIFFC report rows from the individual fire IDs named inside them so a partially matched grouped row cannot hide a missing fire. Publish assets before atomically replacing the manifest, validate byte length/SHA-256/schema/version in the browser, and prune superseded assets only after publication.
...
- On 2026-08-15, added BC Wildfire Service exact-ID display-name enrichment. CWFIS remains canonical; a BC `INCIDENT_NAME` is displayed only when it differs from its `FIRE_NUMBER` and exactly matches the same normalized BC CWFIS fire identifier. CIFFC labels no longer become UI/cache display names and remain only Priority matching and audit evidence. This preserves the CIFFC significance filter while making every displayed Canadian name provincially authoritative and sourced.
