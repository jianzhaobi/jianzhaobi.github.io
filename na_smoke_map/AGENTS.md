...
Map CWFIS stages exactly: `OC` → `Out of control`, `BH` → `Being held`, `UC` → `Under control`, and `EX` → `Extinguished`. Natural/human/undetermined causes map from `N`/`H`/`U`; response labels map `FUL`/`MOD`/`MON` to Full/Modified/Monitored response. Retain `Not reported` for unknown values. Display Canadian area in hectares and use `Large · 120+ ha` as the approximate Canadian counterpart to the US 300-acre browsing threshold. Point radius may reuse the compressed US acreage scale after converting hectares internally, but visible Canadian size copy remains in hectares. Point colour uses situation-report/status recency because CWFIS does not provide the WFIGS discovery timestamp; do not describe that colour as discovery age.

### CIFFC Priority matching

`Priority` is Canada's default significance modifier, analogous in interface purpose but not definition to US IMSR. Join each CIFFC priority row to Agency Reported only within the same agency, first using fire-ID tokens in `field_fire_id` against normalized `agency_fire_id` (ignore punctuation and an optional four-digit year prefix). One CIFFC label may contain multiple fire IDs; mark every unambiguous matching Agency Reported record. Use same-agency coordinate proximity only when no ID token resolves. CIFFC priority text may supply a display name, but CWFIS stage, location, size, containment, response, and identifiers remain canonical. Do not publish unmatched CIFFC rows as invented standalone national fires; record matched/unmatched counts in the manifest instead.

### Canadian cache, UI, and interaction

- `scripts/build_canada_wildfire_cache.py` runs hourly in the Pages workflow and publishes `cache/canada-wildfires/manifest.json` plus content-addressed `default` and `catalog` JSON assets. The default asset is the matched CIFFC Priority subset; the catalog is the complete temporally current Agency Reported set. Publish assets before atomically replacing the manifest, validate byte length/SHA-256/schema/version in the browser, prune superseded assets only after publication, and retain a complete prior cache when either official source fails.
- The browser is cache-only and CSP remains `connect-src 'self'`; build-time CWFIS/CIFFC hosts never belong in the page allowlist. Canada is lazy: initial page load remains US and starts only the US WFIGS default/catalog lifecycle. The Canadian manifest/default request begins only when the Canada tab is first selected; its complete catalog is scheduled afterward during idle time and inflated in chunks.
- The Fires drawer exposes `United States` and `Canada` tabs, with United States selected at startup and after unified reset. Switching tabs clears selection and resets the destination to its significant default (no status chip, IMSR or Priority on, Large off, empty search, latest-time sort). US perimeters return to their prior visibility preference when the user comes back from Canada.
- Canada shows `Reported fire locations`, hides the Perimeters checkbox, and renders points only. Its mutually exclusive status chips are Out of control, Being held, Under control, and Extinguished. `Priority` and `Large · 120+ ha` are independent modifiers. Priority shows every match; otherwise local pagination is 50 final matches with exact totals. Search matches the priority name when available or the displayed agency/fire ID; sort toggles between latest report/status time and largest area.
- The map still mirrors the list exactly. Canadian filtering, selection, point popup, source attribution (`NRCan CWFIS · CIFFC`), generation guards, Load more behavior, and missing-location handling reuse the same canonical `fireMirrorRecords`/`canonicalFireRecords` contract as US. A Canadian popup shows reported date, hectares, containment when available, agency/region, fire ID, cause, response, status update, Priority date when applicable, and `Reported fire location` data availability.
...
- Canadian CWFIS points are agency-reported fire locations, not guaranteed ignition coordinates, and this implementation has no Canadian perimeter source. The 120-hectare threshold, report-recency colour, and converted point-radius scale are browsing aids; visible Canadian size remains in hectares. CIFFC Priority is a situation-report designation rather than the same operational definition as US IMSR, and a priority label can group multiple Agency Reported IDs.
...
- `cache/canada-wildfires/manifest.json`: an empty development fallback; production deployment replaces it and adds the Canadian Priority/default and Agency Reported/catalog assets.
