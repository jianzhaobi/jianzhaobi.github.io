...
- The overlay is North America wide (GOES East/West coverage) and its data is fetched once and cached in memory for the session (`hmsFeatures`); toggling off clears the drawn layers but keeps the cache so re-enabling is instant.
...
- Show the matching total in the list status when the first page did not already capture everything, e.g. `50 of 24,344 wildfires shown`. `loadFireDatabaseTotal()` runs a `returnCountOnly=true` companion query with the same service and where clause, cached per service+where; it is not awaited (the list shows immediately and the `of N` clause fills in when the count returns), and a failed count silently leaves the plain `N shown`. `renderFireListStatus()` composes the line so it can be re-run when the count resolves.
...
- **List set (`fireMirrorRecords`)** is maintained at the two points where the list DOM mutates — cleared alongside `fireList.replaceChildren()` and extended as each row is appended — so `Load more` adds map markers in lockstep and an aborted load leaves it untouched. It is loaded on startup (`initialize()` calls `loadFireDatabase(true)` even with the drawer closed — the old `fireDrawer.hidden` early-return in `loadFireDatabase()` was removed so the map can populate/refresh without the drawer), on every filter/search/sort change, on `Load more`, and re-loaded on the five-minute cycle **only when `fireFilterStateIsDefault()`** (a `refreshWildfires()` success reloads the default list; non-default filtered/paginated views keep the user's snapshot so their pagination is not disrupted — their geometry still refreshes because `renderFireLayers()` runs on every refresh).
...
- **CX parents**: `renderFireLayers()` draws only the member fires found in memory (`complexMemberRecords()`, an in-memory scan matching `CpxID` to the parent id — never a request); the parent itself is never drawn as an ignition circle. It is represented by its diamond, drawn by `renderComplexMarkers()` from the list's CX rows and positioned at the member fires' bounds center (`complexDisplayLatLng()`), falling back to the raw placeholder point only when no member geometry is in memory.
